...
type AircraftIcons = {
  [K in AircraftIcon]: IAircraftIcon;
};
...
const getAircraftSizeByCoef = (coef: number, strict = false) => {
...
export const aircraftIcons: AircraftIcons = {
  ...
  c68a: {
    icon: 'c68a',
    width: getAircraftSizeByCoef(0.37),
  },
  v10: {
    icon: 'v10',
  ...
  il96: {
    icon: 'il96',
    width: getAircraftSizeByCoef(1.0),
  },
  hunt: {
    icon: 'hunt',
    width: getAircraftSizeByCoef(0.17),
...
const getAircraftIcon = (type: string): IAircraftIcon => {
  switch (type) {
    case 'AS32':
    ...
    case 'VULC':
    case 'YK40':
      return aircraftIcons[type.toLowerCase() as AircraftIcon];
    default:
